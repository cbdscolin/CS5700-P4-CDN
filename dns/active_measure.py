...
class LoadMeasurer:
    ...
    def check_load(self):
        ...
        for rep_index, rep_ip in enumerate(self.geo_ip_locator.replica_IPs):
            replica_ip_ratings_pairs[rep_ip] = 0
        # replica_ip_str = ""
        # # Run scamper command to probe replica IPs
        # for ip in self.geo_ip_locator.replica_IPs:
        #     replica_ip_str += " -i " + ip
        # cmd = os.popen('timeout 3s scamper -c "trace -d 40002 -P TCP "' + replica_ip_str)
        # out = cmd.read()
        # cmd.close()
        ...
        #         elif self.geo_ip_locator.distance_to_replicas[rep_index] > 7000:
        #             replica_ip_ratings_pairs[rep_id] = 0
